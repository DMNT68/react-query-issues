...
export const ListViewInfinite = () => {
  ...
  return (
    <div className="row mt-5">
      <div className="col-8">
        {issuesQuery.isLoading ? <LoadingIcon /> : <IssueList issues={issuesQuery.data?.pages.flat() || []} state={state} onStateChange={(newState) => setState(newState)} />}
        <button disabled={!issuesQuery.hasNextPage} className="btn btn-outline-primary mt-2" onClick={() => issuesQuery.fetchNextPage()}>
          {!issuesQuery.isFetching ? 'Load More...' : 'Loading...'}
        </button>
      </div>
